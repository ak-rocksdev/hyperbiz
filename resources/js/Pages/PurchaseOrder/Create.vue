...
const form = ref({
    ...
    exchange_rate: 1,
    tax_rate: props.defaultTaxEnabled ? props.defaultTaxPercentage : 0,
    discount_amount: 0,
    shipping_cost: 0,
    notes: '',
    items: [],
...
const submitForm = (action = 'draft') => {
    ...
    axios.post('/purchase-orders/api/store', payload)
        .then(response => {
            Swal.fire({
                toast: true,
                position: 'top-end',
            ...
            router.visit(route('purchase-orders.list'));
        })
        .catch(err => {
                                ...
                                <table class="table table-sm">
                                    <tbody>
                                        <tr v-for="product in filteredProducts" :key="product.id"
                                            ...
                                            <td class="w-10">
                                                <i class="ki-filled ki-plus-squared text-primary"></i>
                                            </td>
                                        </tr>
