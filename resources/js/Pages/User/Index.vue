...
// Fetch users with filters (for filter/pagination changes - updates URL)
const fetchUsers = () => {
    const params = {
...
// Reload users data (for CRUD operations - partial reload, no full page refresh)
const reloadUsers = () => {
...
const openEditModal = async (user) => {
    try {
        ...
        form.value = {
            ...
            password: '',
            password_confirmation: '',
            role: selectedUser.value.roles?.[0] || '', // Get first role
        };
        errors.value = {};
...
// Edit from detail modal (preserve user before closing)
            ...
            <div class="grid gap-5 lg:gap-7.5">
                    ...
                    <div class="card-body p-5">
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                ...
                                <!-- Role Filter -->
                                <select class="select select-sm w-[140px]" v-model="roleFilter">
                                ...
                                <!-- Status Filter -->
                                <select class="select select-sm w-[130px]" v-model="statusFilter">
                ...
                <div class="card">
                    <div class="card-body p-0">
                        <!-- Empty State -->
                        <div v-if="users.length === 0" class="flex flex-col items-center justify-center py-16 text-gray-400">
                            ...
                            <table class="table-auto w-full">
                                <thead>
                                    <tr class="bg-gray-50 border-b border-gray-200">
                                        <th class="text-left p-4 font-semibold text-gray-600 text-xs uppercase tracking-wider">User</th>
                                        <th class="text-left p-4 font-semibold text-gray-600 text-xs uppercase tracking-wider">Role</th>
                                        <th class="text-center p-4 font-semibold text-gray-600 text-xs uppercase tracking-wider w-[100px]">Status</th>
                                        <th class="text-left p-4 font-semibold text-gray-600 text-xs uppercase tracking-wider w-[150px]">Registered</th>
                                        <th class="text-center p-4 font-semibold text-gray-600 text-xs uppercase tracking-wider w-[100px]">Actions</th>
                                    </tr>
                                </thead>
                                    ...
                                    <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="p-4">
                                            <div class="flex items-center gap-3">
                                                <div v-if="user.profile_photo_path" class="w-10 h-10 rounded-full overflow-hidden border border-gray-200">
                                                    <img :src="'/storage/' + user.profile_photo_path" :alt="user.name" class="w-full h-full object-cover">
                                                </div>
                                                <div v-else class="w-10 h-10 rounded-full bg-primary/10 text-primary font-semibold flex items-center justify-center text-sm">
                                        ...
                                        <td class="p-4">
                                            <div class="flex flex-wrap gap-1">
                                                ...
                                                <span v-if="!user.roles?.length" class="text-gray-400 text-sm">No role</span>
                                            </div>
                                        </td>
                        ...
                        <!-- Pagination -->
                        <div v-if="users.length > 0" class="flex flex-col md:flex-row gap-4 text-gray-600 text-sm font-medium justify-between items-center p-4 border-t border-gray-200 bg-gray-50">
                            <div class="flex items-center gap-2">
                                <span class="text-gray-500">Show</span>
                                <select class="select select-sm w-20" v-model.number="perPage">
                                    <option v-for="option in perPageOptions" :key="option" :value="option">{{ option }}</option>
                                </select>
                                <span class="text-gray-500">of {{ totalResults }} users</span>
                            </div>

                            <div class="pagination flex items-center gap-1">
                                <button class="btn btn-sm btn-icon btn-light" :disabled="currentPage <= 1" @click="goToPage(currentPage - 1)">
                                ...
                                <button class="btn btn-sm btn-icon btn-light" :disabled="currentPage >= lastPage" @click="goToPage(currentPage + 1)">
